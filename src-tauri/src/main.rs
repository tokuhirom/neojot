...
fn tauri_git_init() -> Result<(), String> {
    git_init().map_err(|e| format!("Failed to initialize repository: {}", e))?;
    Ok(())
}

#[tauri::command]
fn tauri_git_add_commit_push() -> Result<(), String> {
    git_add_commit_push().map_err(|e| format!("Failed to operate repository: {}", e))?;
    Ok(())
}

#[tauri::command]
fn tauri_get_commits_by_day(year: i32, month: u32) -> Result<HashMap<u32, Vec<String>>, String> {
    get_commits_by_day(year, month).map_err(|e| format!("Failed to operate repository: {}", e))
}

#[tauri::command]
fn open_url(url: String) -> Result<(), String> {
    let valid_url = Url::parse(&url.clone()).map_err(|_| format!("Invalid URL: {}", url))?;

    // httpsかhttpで始まるかチェック
    match valid_url.scheme() {
        "http" | "https" => {
            open::that(url.clone()).map_err(|err| format!("Cannot open '{}': {:?}", url, err))
        }
        _ => Err(format!(
...
fn load_file_item(filename: String) -> Result<FileItem, String> {
    ...
    let metadata =
        fs::metadata(path.clone()).map_err(|e| format!("Failed to read file metadata: {}", e))?;
            ...
            app.on_menu_event(move |app, event| {
                if event.id().0 == "new_excalidraw" {
                    match create_new_excalidraw() {
                        Ok(fname) => {
                            ...
                            if let Err(err) = app.emit("do_new_excalidraw", fname) {
                                log::error!(
                                    "Cannot emit message for action 'do_new_excalidraw': {:?}",
                                    err
                                );
                            }
                        }
                    ...
                    if let Err(err) = app.emit(&action_name, "DUMMY".to_string()) {
                        log::error!(
                            "Cannot emit message for action '{}': {:?}",
                            action_name,
                            err
                        );
                    }

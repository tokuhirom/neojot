<script lang="ts">
    ...
    let dataFileMap: Record<string, FileItem> = {};
    ...
    function generateCalendar(year: number, month: number): CalendarDay[][] {
        ...
        while (dayCounter <= numberOfDaysInMonth) {
            ...
            week.push({ day: dayCounter, date: new Date(year, month - 1, dayCounter) });
            dayCounter++;
        }
        ...
        <table>
            {#each calendars as week}
                ...
                {#each week as day}
                    <td class="day-cell">
                        {#if day.day!==null}
                            <div class="day {day.date && day.date.getDay() === 0 ? 'sunday' : day.date && day.date.getDay() === 6 ? 'saturday' : ''}">{day.day}</div>
                            {#if calendarData}
                                {#each (calendarData[day.day] || []) as filename}
                                    {#if fileMap[filename]}
                                        <button on:click={() => openFile(fileMap[filename].fileItem)}>{fileMap[filename].fileItem.title}</button>
                                    {/if}
                                {/each}
                            {/if}
                        {/if}

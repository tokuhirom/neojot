...
export const gotoLinkPlugin = function (searchItem: (keyword: string) => void) {
    return ViewPlugin.fromClass(GotoLinkPlugin, {
        ...
        eventHandlers: {
            ...
            mousedown: (event: MouseEvent, _view: EditorView) => {
                ...
                if (target.matches('.goto-link')) {
                    // ここでリンクのクリックイベントを処理
                    console.log(`goto link clicked! ${target.innerText}`);
